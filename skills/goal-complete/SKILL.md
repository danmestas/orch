...
- Mid-pursuit status checks → use `orch-goal-status` instead.
...
- Spec: [`docs/goal-management.md`](https://github.com/danmestas/sesh/blob/main/docs/goal-management.md) in the sesh repo.
- Hooks: `orch-goal-stop-account.sh` and `orch-goal-session-context.sh` in this repo's `hooks/` directory automate token accounting and context injection.
- Bin: `orch-goal-pursue` and `orch-goal-status` in this repo's `bin/` directory are the operator-facing CLIs.
- Reference continuation prompt: `references/orch-goal-continuation.md` for the harness-side prose the spec calls out.
