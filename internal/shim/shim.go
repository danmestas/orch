...
// Synadia §3.1 service name (constant across all agents). Callers
// discover via `$SRV.INFO.agents`.
const serviceName = "agents"

// §3.3 prompt queue group: all instances of the same identity (same
// agent + owner + endpoint subject) share work via this group.
const promptQueueGroup = "agents"

// §8.7 status endpoint queue group; spec says "same as prompt".
const statusQueueGroup = "agents"
...
func (s *shim) start() error {
	...
	if err := svc.AddEndpoint("prompt",
		...
		micro.WithEndpointQueueGroup(promptQueueGroup),
	...
	if err := svc.AddEndpoint("status",
		...
		micro.WithEndpointQueueGroup(statusQueueGroup),
	); err != nil {
		_ = svc.Stop()
