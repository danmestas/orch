...
- `orch/docs/{multi-executor-workers,working-with-sesh}.md` (and `nats-bridge.md` for historical context)
...
| 3 | Cross-leaf pub/sub | pub-on-leaf1-sub-on-leaf2 via hub | (T3.2 legacy-bridge case retired in #94) |
...
| 6 | Subject namespacing + bridge | (retired in #94 — legacy bridge subjects are gone) | n/a |
...
2. (Resolved in orch#94: the legacy `orch-nats-bridge-in` daemon was retired.
   Bus coverage is now via `orch-agent-shim` on the Synadia path, exercised
   in Group 7.)
